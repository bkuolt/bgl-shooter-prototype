...
struct Cluster {
  ...
  void CreateClusters(dvis_t *vis) {
    ...
    for (size_t i = 0; i < vis->numclusters; ++i) {
      clusters[i].visible_clusters =
          DecompressPVS(data + vis->bitofs[i][0], vis->numclusters);
    }
  }
  ...
  static std::pair<int32_t, int32_t> FindNodeLeaf(void) {
    vec3_t v = {camera.getPosition()[X], camera.getPosition()[Y],
  ...
  const char *GetVisibiltyString(void) {
    ...
    int32_t count = (cluster == -1)
                        ? vis->numclusters
                        : clusters[cluster].getVisibleClustersCount();
    float position[3] = {camera.getPosition()[X], camera.getPosition()[Y],
                         camera.getPosition()[Z]};
    static char buffer[100];

    // Formatiert String
    sprintf(buffer,
            "Position(%4i|%4i|%4i), Node: %4i, Leaf: %4i, Cluster %4i, Visible "
            "Clusters: %4i",
            (int)position[X], (int)position[Y], (int)position[Z], node, leaf,
            cluster, count);
    return buffer;
  }
  ...
  void RenderVisibleClusters(void) {
    ...
    if (c == -1) {
      ...
      if (clusters[c].getVisibleClustersCount() > 0) {
        ...
        ::RenderAllClusters(); // Work-Around: bei sehr,sehr wenigen Clustern
                               // betr�gt die Sichtbarkeit 0!?
      }
    }
  ...
  const char *GetCollisionDetetctionString(void) {
    ...
    if (detect_collisions) {
      // Basisinformation
      sprintf(auxbuffer,
              "Checked Solid Brushes: %4i, Checked Planes: %4i, Solid Brush "
              "Collision: %s",
  ...
  /**
   * @brief Überprüft,ob es eine Kollision der Kaerma-AABB mit einer der Brushes
   * im Level gibt
   */
   ...
   * @brief Kernstück der Wandkollisonserkennung:
            Überprüft, ob Kamera-AABB die W�nde des Levels schneiden schneiden
   W�RDEN,wenn die Kamera um den angegeben Vektor verschoben werden w�rde.
   */
