...
public:
  Buffer(GLenum target)
      : data(NULL), count(0), target(target){glGenBuffers(1, &id)}

        Buffer(GLenum target, GLsizei count)
      : data(NULL), count(count), target(target) {
    glGenBuffers(1, &id) glBindBuffer(target, id);
    glBufferData(target, count * sizeof(GLtype) * N, NULL, usage);
    glBindBuffer(target, 0);
  }

  Buffer(GLenum target, const Buffer &rhs)
      : data(NULL), count(0), target(target) {
    glGenBuffers(1, &id) operator=(rhs);
  }

  ~Buffer(void) { glDeleteBuffers(1, &id); }
...
public:
  ...
  void disable(void) const { glDisableClientState(GL_VERTEX_ARRAY); }
};
