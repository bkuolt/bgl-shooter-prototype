...
Camera::Camera(const Vector &position, const Vector &focus,
               const ViewingFrustum &frustum)
    : position(position), focus(focus), frustum(frustum),
      angle(0.0f) { // Angepasste Einstellungen
  current_camera = this;
  calculateBoundingSphere();
}

Camera::Camera(void)
    : position(0.0f, 0.0f, 0.0f), focus(0.0f, 0.0f, -1.0f),
      angle(0.0f) { // OpenGL Standardeinstellungen
  current_camera = this;
  calculateBoundingSphere();
}

Camera::Camera(const ViewingFrustum &frustum)
    : position(0.0f, 0.0f, 0.0f), focus(0.0f, 0.0f, -1.0f), frustum(frustum),
      angle(0.0f) {
  current_camera = this;
  calculateBoundingSphere();
}
...
void Camera::set(void) const {
  ...
  gluLookAt(position[X], position[Y], position[Z], // Position
            position[X] + focus[X], position[Y] + focus[Y],
            position[Z] + focus[Z], // Fokus
            0.0f, 1.0f, 0.0f);
}
...
void Camera::moveLeft(float step) {
  translate(CrossProduct(Vector(0.0f, 1.0f, 0.0f), getFocus()).normalize() *
            fabs(step));
}
 ...
 */
void Camera::moveUp(float step) { translate(Vector(0.0f, fabs(step), 0.0f)); }
...
void Camera::moveForward(float step) {
  translate(Vector(getFocus()[X], 0.0f, getFocus()[Z]).normalize() *
            fabs(step));
}
 ...
 */
void Camera::rotateUp(float step) { rotate(X, fabs(step)); }

void Camera::rotateDown(float step) { rotate(X, -fabs(step)); }

/**
 * @brief Rotiert Kamera nach links bzw. nach rechts
 */
void Camera::rotateLeft(float step) { rotate(Y, fabs(step)); }

void Camera::rotateRight(float step) { rotate(Y, -fabs(step)); }
...
Vector Camera::getVisiblePosition(void) const {
  // Vector offset(0,-2.5,0); // Offset sorgt dafür,dass es zu keinen
  // Clippingfehlern mit dem Bodem kommt
  return position + Vector(focus).normalize() *
                        frustum.near; // Könnte Probleme bei der AABB
                                      // Kollisionserkennung geben!
}
...
static void CameraCallback(int key) {
  ...
  // Rotiert Kamera
  case Left:
    camera.rotateLeft(step);
    break;
  case Right:
    camera.rotateRight(step);
    break;
  case Down:
    camera.rotateDown(step);
    break;
  case Up:
...
void CameraCallback(int x, int y) {
  ...
  if (x > glutGet(GLUT_WINDOW_WIDTH) * 0.75f ||
      x < glutGet(GLUT_WINDOW_WIDTH) * 0.25f ||
      y > glutGet(GLUT_WINDOW_HEIGHT) * 0.75f ||
      y < glutGet(GLUT_WINDOW_HEIGHT) * 0.25f)
    SetCursorPos(glutGet(GLUT_WINDOW_WIDTH) / 2,
