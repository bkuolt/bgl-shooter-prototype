...
public:
  ...
  Vector(void) { values[X] = values[Y] = values[Z] = 0.0f; }

  float &operator[](Axis axis) { return values[axis]; }

  const float &operator[](Axis axis) const { return values[axis]; }
  ...
  Vector operator-(const Vector &rhs) { return Vector(*this) -= rhs; }

  Vector operator+(const Vector &rhs) { return Vector(*this) += rhs; }

  Vector operator*(float k) const { return Vector(*this) *= k; }

  Vector operator*(float k) { return Vector(*this) *= k; }

  float length(void) const {
    return std::sqrt(values[X] * values[X] + values[Y] * values[Y] +
                     values[Z] * values[Z]);
  }

  Vector &normalize(void) { return *this *= (1.0f / length()); }
  ...
  ==============================================*/
  const float *begin(void) const { return values; }

  float *begin(void) { return values; }

  const float *end(void) const { return values + 3; }

  float *end(void) { return values + 3; }
  ...
  bool operator==(const Vector &rhs) const {
    return rhs[X] == values[X] && rhs[Y] == values[Y] && rhs[Z] == values[Z];
  }
};
//----------------------------------------------------------------
...
inline Vector CrossProduct(const Vector &a, const Vector &b) {
  return Vector(a[1] * b[2] - b[1] * a[2], a[2] * b[0] - b[2] * a[0],
                a[0] * b[1] - b[0] * a[1]);
}
