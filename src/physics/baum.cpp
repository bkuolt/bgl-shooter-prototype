...
struct Cluster {
    ...
    void findContainingFaces(void) {
        ...
        for (size_t i = 0; i < leafs.size(); ++i) {
            if (leafs[i].cluster == static_cast<short>(id)) {
...
void CreateClusters(dvis_t* vis) {
    ...
    spdlog::info("  Decompressing PVSs...");
...
static std::pair<int32_t, int32_t> FindNodeLeaf(void) {
    vec3_t v = {camera.getPosition()[X], camera.getPosition()[Y], camera.getPosition()[Z]};
...
const char* GetVisibiltyString(void) {
    ...
    std::snprintf(buffer, sizeof(buffer),
                  "Position(%4i|%4i|%4i), Node: %4i, Leaf: %4i, Cluster %4i, Visible Clusters: %4i",
                  static_cast<int>(position[X]), static_cast<int>(position[Y]), static_cast<int>(position[Z]), node,
                  leaf, cluster, count);
    return buffer;
}
...
const char* GetCollisionDetetctionString(void) {
    ...
    if (detect_collisions) {
        std::snprintf(auxbuffer, sizeof(auxbuffer),
                      "Checked Solid Brushes: %4i, Checked Planes: %4i, Solid Brush Collision: %s", checked_brushes,
                      checked_planes, (collision) ? "Yes" : "No");
        std::snprintf(buffer, sizeof(buffer), "%s", auxbuffer);
    } else {
        std::snprintf(buffer, sizeof(buffer), "Collision Detection disabled");
    }
    return buffer;
}
...
bool WouldHaveWallCollision(const Vector& v) {
    ...
    bool result = HasWallCollision();
    camera.position -= v;
    camera.sphere.translate(v * -1.0f);
    return result;
}
