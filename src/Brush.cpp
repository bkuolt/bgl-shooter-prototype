...
Brush::Brush(const dbrush_t &brush, const dbrushside_t *brushsides,
 ...
 * @note "Alternatively since it's just plane box intersection you can find the
 * signed distance to the plane for every point on the box (+-Xx,+-Xy,+-Xz)and
 * if all of those signs are the same then it lies completely on one side,
...
bool Brush::intersects(const BoundingSphere &sphere) const {
  ...
  if (contents & CONTENTS_SOLID) {
    ...
    // (2) Überprüft, ob Abstand des Mittelpunkts der Kugel zu allen Ebenen
