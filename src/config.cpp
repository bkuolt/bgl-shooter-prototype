...
static float GetPrivateProfileFloat(const char* lpAppName,const char* lpKeyName,float inDefault,const char* lpFileName)
{
    char buffer[100];
    char _default[100];

    sprintf(_default,"%f",inDefault);
    GetPrivateProfileString(lpAppName,lpKeyName,_default,buffer,100,lpFileName);
    return atof(buffer);
}
...
int GetConfigurationInt(enum Configuration configuration)
{
    ...
    {
        case Width:         return (width  == -1) ? glutGet(GLUT_SCREEN_WIDTH)  : width;
        case Height:        return (height == -1) ? glutGet(GLUT_SCREEN_HEIGHT) : height;
        case Windowed:      return windowed;
        case PlayAudio:     return audio;
        case DrawSkybox:    return skybox;
        case AnimateSkybox: return animated;
        case AnimateWater:  return water;
        case DrawTextures:  return textures;
        case DrawWater:     return renderwater;
        case AnisotropicFilter: return AF;
    }
}
...
std::string GetConfigurationString(enum Configuration configuration)
{
    switch(configuration)
    {
        case TextureFormat: return (HD) ? ".tga" : ".wal";
        case DataPath: return (data == "local") ? GetProgrammPath() + "data\\" : data;
        case Level:    return level + ".bsp";
     }
}
...
float GetConfigurationFloat(enum Configuration configuration)
{
    switch(configuration)
    {
        case RotationStep: return rotation;
        case MoveStep:     return translation;
        case MoveUpStep:   return up;
...
void LoadConfiguration(void)
{
    ...
    // String
    char buffer[2][100];
    GetPrivateProfileString("BGL","Level",     "base2",buffer[0],100,file.c_str());
    GetPrivateProfileString("BGL","DataFolder","local",buffer[1],100,file.c_str());
