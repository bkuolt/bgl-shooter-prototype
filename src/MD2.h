...
inline std::string GetPath(const std::string &name) { return GetPath() + name; }
  ...
  void load(const std::string filename) {
    ...
    for (int i = 0; i < header.num_frames; ++i) {
      ...
      file.seekg(header.ofs_frames + (i * header.framesize) + 40,
                 std::ios_base::beg);
      file.read(reinterpret_cast<char *>(frames[i].verts),
                header.num_xyz * sizeof(vertex_t));
    }
    ...
    for (size_t f = 0; f < keyframe_count; ++f) {
      ...
      for (size_t v = 0; v < vertex_count; ++v)
        for (size_t c = 0; c < 3; ++c)
          keyframes[f].vertices[v][c] =
              (frames[f].verts[v].v[c] * frames[f].scale[c]) +
              frames[f].translate[c];
    }
    ...
    /*
    ======================================================
    L�dt MD2-Texturkoordinaten
    ======================================================*/
    tex_coord_t *sts = new tex_coord_t[header.num_st];

    file.seekg(header.ofs_st, std::ios_base::beg);
    file.read(reinterpret_cast<char *>(sts),
              sizeof(tex_coord_t) * header.num_st);
    ...
    file.read(reinterpret_cast<char *>(triangles),
              sizeof(triangle_t) * header.num_tris);
    ...
    for (size_t t = 0; t < header.num_tris; ++t)
      for (size_t v = 0; v < 3; ++v) {
        ...
        texture_coords[(3 * t) + v][0] =
            ((float)sts[index].u / (float)header.skinwidth);
        texture_coords[(3 * t) + v][1] =
            ((float)sts[index].v / (float)header.skinheight);
      }
...
private:
   ...
   */
  void interpolate(double **vertices, size_t first, size_t second,
                   float factor) {
    for (size_t v = 0; v < vertex_count; ++v)
      for (size_t c = 0; c < 3; ++c)
        vertices[v][c] = keyframes[first].vertices[v][c] +
                         (keyframes[second].vertices[v][c] -
                          keyframes[first].vertices[v][c]) *
                             factor;
  }
...
public:
  ...
  void normalize(void) {
    ...
    for (size_t c = 0; c < 3; ++c)
      center[c] =
          extrema[c][MIN] + ((extrema[c][MAX]) - (extrema[c][MIN])) / 2.0f;

    // Debug: Extremwerte ausgeben
    printf("max: %f,%f,%f\n"
           "min: %f,%f,%f\n",
           extrema[0][MAX], extrema[1][MAX], extrema[2][MAX], extrema[0][MIN],
           extrema[1][MIN], extrema[2][MIN]);
  ...
  /**
   * @brief Gibt Information �ber ablaufende Animation zurück.
   */
