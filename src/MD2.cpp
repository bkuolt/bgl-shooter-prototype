 ...
 */
void MD2::render(double** const vertices) const {
	glPushMatrix();
 ...
 *
 */
void MD2::createAnimationList(void) {
 ...
 */
void MD2::interpolate(double** vertices, size_t first, size_t second, float factor) {
	for (size_t v = 0; v < vertex_count; ++v)
		for (size_t c = 0; c < 3; ++c)
 ...
 */
void MD2::start(size_t animation, size_t fps, bool do_repeat) {
	started = true;
	repeat = do_repeat;
	this->fps = fps;
	current_frame = 0;
 ...
 */
bool MD2::isRunning(void) const {
	return started;
}

int MD2::GetCurrentAnimation(void) const {
...
MD2::WeaponWeapon(void)
	: active(false), fight(false) {
}

MD2::WeaponWeapon(const std::string& file)
	: weapon(file), active(false), fight(false) {
}

void MD2::Weapon(const std::string& file) {
	weapon.load(file);
}
 ...
 */
void MD2::Weapon::put(void) {
	putaway();
}

void MD2::Weapon::putaway(void) {
	if (active) {
		active = false;
...
void MD2::Weapon::startShooting(void) {
	fight = true;
	gun.play(true);
	gun.setSpeed(2);
	gun.setVolume(1.0);
...
void MD2::Weapon::stopShooting(void) {
	fight = false;
	gun.stop();
	weapon.start(Idle, 10, true);
}

void MD2::Weapon::idle(void) {
	weapon.start(Idle, 10, true);
}